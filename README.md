...
A five-part series on how large language models are served in production, written for engineers and architects who work with these systems but have not built one. It starts from the hardware, ends at the decisions a customer actually asks about, quantization, structured outputs, tool calling and the tail latency of agents, and every number in it is one you can derive with a pen.
...
Four formulas carry the series:
...
- **Ridge point** = FLOPS ÷ bandwidth (about 295 operations per byte on an H100 in FP16, 591 in FP8): below it a workload is memory-bound, above it compute-bound

With those, a benchmark table becomes something you can rebuild from first principles, every optimization in the field can be classified by which limit it attacks, and an autoscaling setting becomes a statement about cold starts. The final part puts the model to work on the questions that decide deployments: which quantization format helps which phase, how a JSON schema becomes a token mask, why an agent's first token gets slower every turn, and why one user action in ten hits the tail when every model call is fast. The series is vendor-neutral; every figure is cited to NVIDIA datasheets, model cards, or the original papers.
...
| 4 | Quantization, Structured Outputs and Tool Calling, from the Physics | The roofline and the ridge point; attention's quadratic term; chunked prefill as a dial; W8A8 vs W4A16 and which phase each helps; the Llama 3.1 quantization study; constrained decoding; the tool-calling loop, parsers and failure modes; tail-at-scale arithmetic for agents; the diagnostic tree | [Inference-Fundamentals-Part-4.pdf](Inference-Fundamentals-Part-4.pdf) |

Read 1A and 1B first; they stand on their own. Parts 2, 3 and 4 build on them. Each part ends with a short self-test and a Sources section.

## Sources

Every technical claim is cited at the end of the part where it appears: NVIDIA product pages for H100, H200, B200 and A100; the Llama 3 configuration and paper; the DeepSeek-V3 technical report; Kaplan et al. on scaling laws; the Orca, PagedAttention, SGLang, speculative decoding, Medusa, EAGLE and lookahead decoding papers; NVIDIA's Dynamo announcement; the roofline paper; vLLM source and documentation; the GPTQ, AWQ, SmoothQuant and XGrammar papers and the Llama 3.1 quantization study; Dean and Barroso's "The Tail at Scale"; DistServe; and the release announcements from Meta, Mistral, Alibaba, DeepSeek and OpenAI.
